...
read choice
    if [ $choice -eq 1 ] ; then
        ...
        while :; do cd $root/nadekoc/output && dotnet NadekoBot.dll ; youtube-dl -U && cd $root && wget -N https://raw.githubusercontent.com/epikgallavoir/bashdeko/main/n-download.sh && bash "$root/n-download.sh"; sleep 5s; done
        echo "Done"
    elif [ $choice -eq 2 ] ; then
        echo ""
        echo "Running nedakoc without auto update. Please wait. . ."
        while :; do cd "$root/nadekoc/output" && dotnet NadekoBot.dll && youtube-dl -U; sleep 5s; done
        echo "Done"
